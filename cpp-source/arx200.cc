#pragma hls_top
unsigned short minhash(unsigned short set1 [200], unsigned short set2 [200]){
	...
	#pragma hls_unroll yes
        for ( unsigned short k = 0; k<99; k++){
            unsigned short hash = (set1[k] + b[j]) & p;
            if (hash < minhash_value){
                minhash_value = hash;
            }
        }
	unsigned short hash = (set1[99] + b[j]) & p;
	if (hash < minhash_value){
	    minhash_value = hash;
	}
    ...
    for ( unsigned short j = 0; j<number_of_hashes; j++){
        unsigned short minhash_value = p;
	#pragma hls_unroll yes
        for ( unsigned short k = 100; k<199; k++){
            unsigned short hash = (set1[k] + b[j]) & p;
            if (hash < minhash_value){
                minhash_value = hash;
            }
	...
	unsigned short hash = (set2[199] + b[j]) & p;
        if (hash < minhash_value){
            minhash_value = hash;
        }
